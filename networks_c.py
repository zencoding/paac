    ...
    def __init__(self, conf):
        ...
        with tf.device(self.device):
            with tf.name_scope(self.name):
                ...
                self.input_ph = tf.placeholder(tf.float32, [None, 4], name='input')
                self.selected_action_ph = tf.placeholder("float32", [None, self.num_actions], name="selected_action")
                self.input = self.input_ph  # tf.scalar_mul(1.0/255.0, tf.cast(self.input_ph, tf.float32))
    ...
    def __init__(self, conf):
        ...
        with tf.device(self.device):
            with tf.name_scope(self.name):
                ...
                _, _, fc1 = fc('fc1', self.input, 10, activation="relu")

                _, _, fc2 = fc('fc2', fc1, 8, activation='relu')

                _, _, fc3 = fc('fc3', fc2, 6, activation='relu')

                # _, _, fc4 = fc('fc4', fc3, 2, activation='relu')
